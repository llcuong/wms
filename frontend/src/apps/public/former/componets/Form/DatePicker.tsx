        ...
        <div className={`flex flex-col gap-1.5 ${className}`} ref={pickerRef}>
          ...
          <span className={value ? 'text-gray-900' : 'text-gray-400'}>
            {value || formatPlaceholder()}
                ...
                {isOpen && !disabled && (
                    <div
                        className="absolute z-50 w-full min-w-100 mt-1 bg-white border border-gray-300 rounded-lg shadow-lg p-4">
                        {type === 'day' && (
                            <>
                    ...
                    {viewDate.toLocaleDateString('en-US', {month: 'long', year: 'numeric'})}
                        ...
                        {type === 'month' && (
                            <>
                                <div className="flex items-center justify-between mb-3">
                                    <button
                                        onClick={() => setViewDate(new Date(viewDate.getFullYear() - 1, 0))}
                                        className="p-1 hover:bg-gray-100 rounded"
                                    >
                                        ←
                                    </button>
